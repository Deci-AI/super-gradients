...
class LightweightDEKRHead(nn.Module):
    """
    ...
    def __init__(
        self, in_channels: int, num_joints: int, heatmap_channels: int, offset_channels_per_joint: int, activation: Type[nn.Module], upscale_factor: int = 1
    ):
        """

        :param in_channels: Number of input channels.
        :param num_joints: Number of joints to regress.
        ...
        self.heatmap = nn.Sequential(
            ...
            nn.Conv2d(heatmap_channels, num_joints + 1, kernel_size=1, padding=0),
            nn.Sigmoid(),
        )

        offset_channels_per_joint = num_joints * offset_channels_per_joint
        self.transition_offset = nn.Sequential(
            nn.Conv2d(in_channels, offset_channels_per_joint, kernel_size=1, padding=0, bias=False),
            nn.BatchNorm2d(offset_channels_per_joint),
            activation(inplace=True),
            nn.UpsamplingBilinear2d(scale_factor=upscale_factor) if upscale_factor > 1 else nn.Identity(),
        )

        self.offset_heads = nn.ModuleList([nn.Conv2d(offset_channels_per_joint, 2, kernel_size=1, padding=0) for _ in range(num_joints)])
