...
def iterate_over_obb_detection_predictions_in_flat_format(predictions: NumpyArrayOrTensor, batch_size: int):
    ...
    if len(predictions.shape) != 2 or predictions.shape[1] != 8:
        raise ValueError(f"predictions must be a tensor or numpy array of shape (num_predictions, 8), got {predictions.shape}")
...
def iterate_over_obb_detection_predictions_in_batched_format(
    ...
    """
    Iterate over OBB detection predictions in batched format.
    This method is suitable for iterating over predictions of object detection models exported to ONNX format
    with postprocessing. An exported object detection model can have 'flat' or 'batched' output format.
    ...
    :param predictions:    A tuple of (num_detections, bboxes, scores, labels)
           ...
           bboxes:         A 3D array of shape (batch_size, max_detections, 5) containing bounding boxes in format (cx, cy, w, h, r)
