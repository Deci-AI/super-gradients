...
class QuantBottleneck1(SGQuantMixin):
    ...
    def __init__(self, bottleneck: Bottleneck) -> None:
        ...
        self.bottleneck.shortcut.add_module("residual_quantizer", quant_nn.TensorQuantizer(quant_nn.QuantConv2d.default_quant_desc_input))
...
class QuantBottleneck2(SGQuantMixin):
    ...
    def from_float(cls, float_instance: Bottleneck, **kwargs):
        float_instance.shortcut.add_module("residual_quantizer", quant_nn.TensorQuantizer(quant_nn.QuantConv2d.default_quant_desc_input))
...
def sg_resnet50_custom_quantization():
    ...
    mappings = {
        Bottleneck: QuantizedMetadata(
            float_source=Bottleneck, quantized_target_class=quantized_type, action=QuantizedMetadata.ReplacementAction.QUANTIZE_CHILD_MODULES_THEN_REPLACE
        ),
        nn.AdaptiveAvgPool2d: QuantizedMetadata(
            float_source=nn.AdaptiveAvgPool2d, quantized_target_class=None, action=QuantizedMetadata.ReplacementAction.SKIP
        ),
    }
