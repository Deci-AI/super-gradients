...
class STDCBackbone(AbstractSTDCBackbone):
    def __init__(
        self, block_types: list, ch_widths: list, num_blocks: list, stdc_steps: int = 4, in_channels: int = 3, out_down_ratios: Union[tuple, list] = (32,)
    ):
        """
        ...
        for block_type, width, blocks in zip(block_types, ch_widths, num_blocks):
            ...
            self.stages[block_name] = self._make_stage(
                in_channels=in_channels, out_channels=width, block_type=block_type, num_blocks=blocks, stdc_steps=stdc_steps
            )
    ...
    def _make_stage(self, in_channels: int, out_channels: int, block_type: str, num_blocks: int, stdc_steps: int = 4):
        """
