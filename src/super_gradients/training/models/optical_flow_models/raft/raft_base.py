...
class Encoder(nn.Module, SupportsReplaceInputChannels):
    ...
    def _make_layer(self, dim: int, stride: int = 1):
        if self.block == "BottleneckBlock":
            layer1 = BottleneckBlock(self.in_planes, dim, self.norm_fn, stride=stride)
            layer2 = BottleneckBlock(dim, dim, self.norm_fn, stride=1)
        elif self.block == "ResidualBlock":
...
class ContextEncoder(nn.Module):
    def __init__(
        ...
        self.cnet = Encoder(in_channels=in_channels, in_planes=in_planes, block=block, output_dim=output_dim, norm_fn=norm_fn, dropout=dropout)
...
class FlowIterativeBlock(nn.Module):
    def __init__(self, encoder_params, hidden_dim, flow_params, alternate_corr):
        ...
        self.training_iters = flow_params.training_iters
        self.validation_iters = flow_params.validation_iters
