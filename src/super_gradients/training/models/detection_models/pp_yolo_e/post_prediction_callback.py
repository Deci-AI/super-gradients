...
class PPYoloEPostPredictionCallback(DetectionPostPredictionCallback):
    ...
    def __init__(self, score_threshold: float, nms_threshold: float, nms_top_k: int, max_predictions: int, multi_label_per_box: bool = True):
        """
        :param score_threshold: Results confidence threshold. Results with score lower than score_threshold will not participate in Top-K & NMS
        :param iou: IoU threshold for NMS step.
        :param nms_top_k: Number of results participating in NMS step
    ...
    def forward(self, outputs, device: str):
        ...
        """
        nms_result = []
        # First is model results, second element of tuple is logits for loss computation
        ...
        for pred_bboxes, pred_scores in zip(*predictions):
            ...
            # Filter all results by self.score_threshold
            ...
            # Filter all results by self.nms_top_k
