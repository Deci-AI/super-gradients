...
class COCODetectionDatasetV2(DetectionDataSetV2):
    ...
    def __init__(
        ...
        kwargs['all_classes_list'] = self.classes #TODO fill this
        kwargs['target_format'] = DetectionTargetsFormat.XYXY_LABEL

        target_fields = ["target", "crowd_targets"] if self.with_crowd else ["target"]
        kwargs['annotation_fields_to_subclass'] = target_fields
    ...
    #     """
    #     Loads sample before transform at self.sample_id_to_coco_id[sample_id] as dictionary that holds:
    ...
    #             image coordinates.
    #         "crowd_target": Detection ground truth for crowd, np.array shaped (num_targets, 5),
    #                         format is [class,x1,y1,x2,y2] with image coordinates.
    #         "target_seg": Segmentation map convex hull derived detection target.
    #         "info": Original shape (height,width).
    #         "id": COCO image id
    #
    #     :param sample_id: Sample sample_id
    #     :return: sample as described above, before transforms
    #     """
    #     img = self.get_image(sample_id)
    #     target, crowd_target, target_segmentation, img_info, _, _ = self.annotations[sample_id]
    #     img_id = self.sample_id_to_coco_id[sample_id]
    #
    #     sample = {"image": img, "target": target.copy(), "target_seg": target_segmentation, "info": img_info, "id": np.array([img_id])}
    #     if self.with_crowd:
    #         sample["crowd_target"] = crowd_target.copy()
    #     return sample

    # def get_target(self, sample_id):
    #     return self.annotations[sample_id][0]

    def get_image(self, sample_id):
        if self.cached_imgs is not None:
            pad_img = self.cached_imgs[sample_id]
            _, _, _, _, resized_info, _ = self.annotations[sample_id]
            return pad_img[: resized_info[0], : resized_info[1], :].copy() #TODO: Check if the same
        else:
            return self._load_resized_img(sample_id)
