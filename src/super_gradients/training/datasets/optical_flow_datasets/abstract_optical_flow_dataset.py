...
class AbstractOpticalFlowDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        images = np.transpose(sample.images, (0, 3, 1, 2)).astype(np.float32)
        flow_map = np.transpose(sample.flow_map, (2, 0, 1)).astype(np.float32)
        valid = sample.valid.astype(np.float32)
        return images, (flow_map, valid)
