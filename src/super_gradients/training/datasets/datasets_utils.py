...
class DetectionRandomResizePrePredictionCallback(AbstractPrePredictionCallback):
    """
    Random batch resize pre-prediction callback for object detection.

    Attributes:
        target_size: (int) List of target sizes (rows, cols)
        image_size_steps: (int) Image step sizes as discussed abov (default=32)
        change_frequency: (int) The frequency to apply change in input size.

    """

    def __init__(self, target_size: List[Tuple[int, int]]):
        self.target_size = target_size
        self.interpolation_method = ["nearest", "bilinear", "area"]
        self.interpolation_with_align_corners = {"linear", "bilinear", "bicubic", "trilinear"}
    ...
    def __call__(self, inputs, targets, batch_idx):
        ...
        Args:
            inputs: [B, 3, H,W]
            targets: [B, N, 6], where 6 elements are (image index in batch, x, y, x, y, label)
            batch_idx:

        Returns:

        """
        # Random resize with small deviation of aspect ratio
        n = len(self.target_size)
        i = random.randrange(0, n)
        j = random.choice([max(0, i - 2), i, min(n - 1, i + 2)])

        target_size = self.target_size[i], self.target_size[j]
        interpolation = random.choice(self.interpolation_method)

        input_size = inputs.shape[2:]

        align_corners = False if interpolation in self.interpolation_with_align_corners else None
        inputs = F.interpolate(inputs, size=target_size, mode=interpolation, align_corners=align_corners)
