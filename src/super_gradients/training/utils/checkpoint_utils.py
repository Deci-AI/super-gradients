...
def get_ckpt_local_path(source_ckpt_folder_name: str, experiment_name: str, ckpt_name: str,
    ...
    if model_checkpoints_location == 'local':
        ckpt_local_path = external_checkpoint_path or pkg_resources.resource_filename('checkpoints',
...
def load_pretrained_weights(model: torch.nn.Module, architecture: str, pretrained_weights: str):
    ...
    unique_filename = url.split("https://deci-pretrained-models.s3.amazonaws.com/")[1].replace('/', '_').replace(' ',
                                                                                                                 '_')
