...
def transfer_weights(model: nn.Module, model_state_dict: Mapping[str, Tensor]) -> None:
    ...
    percentage_of_layers_from_checkpoint = transfered_weights / len(model_state_dict)
    percentage_of_layers_in_model = transfered_weights / len(model.state_dict())
    logger.info(
        f"Transfered {transfered_weights} ({(100*percentage_of_layers_from_checkpoint):.2f}%) weights from the "
        f"checkpoint state dict into {100 * percentage_of_layers_in_model:.2f}% of the model layers"
    )
