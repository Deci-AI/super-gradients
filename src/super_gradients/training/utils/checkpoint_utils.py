...
from super_gradients.common import (
...
def get_ckpt_local_path(
    source_ckpt_folder_name: str,
    experiment_name: str,
    ckpt_name: str,
    model_checkpoints_location: str,
    external_checkpoint_path: str,
    overwrite_local_checkpoint: bool,
    ...
    if model_checkpoints_location == "local":
        ckpt_local_path = (
            ...
            or environment_config.PKG_CHECKPOINTS_DIR
            + os.path.sep
            + source_ckpt_folder_name
            + os.path.sep
            + ckpt_name
        )
    ...
    # COPY THE DATA FROM 'S3'/'URL' INTO A LOCAL DIRECTORY
        ...
        model_checkpoints_location.startswith("s3")
        or model_checkpoints_location == "url"
    ):
        # COPY REMOTE DATA TO A LOCAL DIRECTORY AND GET THAT DIRECTORYs NAME
        ckpt_local_path = copy_ckpt_to_local_folder(
            local_ckpt_destination_dir=experiment_name,
        ...
        raise NotImplementedError(
            "model_checkpoints_data_source: "
            + str(model_checkpoints_location)
            + "not supported"
        )
...
def adapt_state_dict_to_fit_model_layer_names(
    ...
    model_state_dict_excluded = {
        k: v for k, v in model_state_dict.items() if not any(x in k for x in exclude)
    }
    new_ckpt_dict = {}
    for (ckpt_key, ckpt_val), (model_key, model_val) in zip(
        source_ckpt.items(), model_state_dict_excluded.items()
    ):
...
def load_checkpoint_to_model(
    ...
    if ckpt_local_path is None or not os.path.exists(ckpt_local_path):
        error_msg = "Error - loading Model Checkpoint: Path {} does not exist".format(
            ckpt_local_path
        )
...
def load_pretrained_weights(
    ...
    unique_filename = (
        url.split("https://deci-pretrained-models.s3.amazonaws.com/")[1]
        .replace("/", "_")
        .replace(" ", "_")
    )
