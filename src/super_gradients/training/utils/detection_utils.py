...
def compute_detection_matching(
    ...
    """
    Match results (NMS output) and the targets (ground truth) with respect to IoU and confidence score.
    ...
    :param crowd_targets:   crowd targets for all images of shape (total_num_crowd_targets, 6)
                            format:     (index, x, y, w, h, label) where x,y,w,h are in range [0,1]
    :param top_k:           Number of results to keep per class, ordered by confidence score
...
def compute_img_detection_matching(
    ...
    # Ignore all but the results that were top_k for their class
...
def get_top_k_idx_per_cls(preds_scores: torch.Tensor, preds_cls: torch.Tensor, top_k: int):
    """Get the indexes of all the top k results for every class
    ...
    :return top_k_idx:     Indexes of the top k results. length <= (k * n_unique_class)
    """
