...
class DetectionTargets:
    """Targets of a given image, along with the format in which this data is stored."""

    data: np.ndarray
    format: ConcatenatedTensorPredictionsFormat

    @property
    def cls(self):
        return self.format.get(name="cls", data=self.data)  # extract the class out of the data tensor

    @property
    def bbox(self):
        return self.format.get(name="bbox", data=self.data)  # extract the bbox out of the data tensor

    @property
    def xyxy(self):  # Example of implementation. I want to get the BoundingBoxFormat, and use it to convert my bbox to xyxy
        return self.format.get(name="bbox").to_xyxy(data=self.bbox)

    # @property
    # def yxyx(self):
    #     return YXYXCoordinateFormat().from_xyxy(bbox=self.xyxy)
    #
    # @property
    # def xywh(self):  # ...
    #     return XYWHCoordinateFormat().from_xyxy(bbox=self.xyxy)
    #
    # @property
    # def cxcywh(self):
    #     return CXCYWHCoordinateFormat().from_xyxy(bbox=self.xyxy)

    def convert(self, output_format: ConcatenatedTensorPredictionsFormat):  # Might not be needed actually
        converter = DetectionOutputAdapter(input_format=self.format, output_format=output_format)
        return DetectionTargets(data=converter(self.data), format=output_format)


targets = DetectionTargets(data=np.array([[0, 0, 10, 15, 2]]), format=ConcatenatedTensorPredictionsFormat(layout=(...)))  # TO FINISH
...
converted_sample = targets.convert(output_format=ConcatenatedTensorPredictionsFormat(layout=(...)))
