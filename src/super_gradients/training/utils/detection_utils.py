...
def xyxy2cxcywh(bboxes):
    """
    Transforms bboxes inplace from xyxy format to centerized xy wh format
...
class DetectionFormatConversionAdapter(nn.Module):
    ...
    def __init__(self, output_format: Union[List[str], List[List[str]]], image_shape: Union[Tuple[int, int], None]):
        """

        :param output_format:
        :param image_shape: Image shape (rows, cols) used to scale boxes to output unit-normalized coordinates.
        """
        super().__init__()
        bbox_format = self.infer_bbox_format(output_format)
    ...
    def forward(self, predictions: List[Tensor]) -> List[Tensor]:
        """

        :param predictions: List[Tensor] - List of tensors of shape [N,6] (x1, y1, x2, y2, conf, cls)
        :return:
        """
        return [self.convert(x) for x in predictions]

    def convert(self, predictions: Tensor):
        """

        :param predictions: (Tensor [N,6]) Tensor of  (x1, y1, x2, y2, conf, cls)
        :return:
        """
        xyxy, rest = predictions[:, 0:4], predictions[:, 4:]
        bboxes = self.convert_boxes_to_output_format(xyxy)  # handles format conversion & normalization
        parts = [bboxes, rest]
        predictions = torch.cat(parts, dim=1)

        return self.rearrange_outputs(predictions, self.output_slices)
    ...
    def infer_bbox_format(cls, output_format: Union[List[str], List[List[str]]]) -> Union[str, None]:
        if isinstance(output_format, str):
            ...
            if len(formats) > 1:
                raise RuntimeError(f"Detected more than one output format requested: {formats}. This is not supported.")

            if len(formats) == 1:
                return formats[0]
        else:
            raise RuntimeError(f"Type {type(output_format)} of output_format ({output_format}) is not supported")
    ...
    def rearrange_outputs(
        cls, predictions: Tensor, output_slices: Union[int, slice, Tuple]
    ) -> Union[Tensor, Tuple[Tensor, ...], Tuple[Tuple[Tensor, ...], ...]]:
        """
        Recursively assemble the output format with respect to output slices

        :param predictions: (Tensor [N,6]) Tensor of  (x1, y1, x2, y2, conf, cls, ...)
        :param output_slices:
        :return:
        """
        if isinstance(output_slices, tuple):
