...
class ImageClassificationPrediction(ImagePrediction):
    ...
    def draw(self, show_confidence: bool = True) -> np.ndarray:
        ...
        cv2.rectangle(image, bg_position, (bg_position[0] + bg_size[0], bg_position[1] + bg_size[1]), label_color,
                      thickness=-1)

        # Draw the label text on the image
        cv2.putText(img=image, text=label_text, org=start_point, fontFace=cv2.FONT_HERSHEY_SIMPLEX,
...
class ImageDetectionPrediction(ImagePrediction):
    ...
    def draw(self, box_thickness: int = 2, show_confidence: bool = True,
             color_mapping: Optional[List[Tuple[int, int, int]]] = None) -> np.ndarray:
    ...
    def show(self, box_thickness: int = 2, show_confidence: bool = True,
...
class ImagesDetectionPrediction(ImagesPredictions):
    ...
    def save(
            self, output_folder: str, box_thickness: int = 2, show_confidence: bool = True,
        ...
        for i, prediction in enumerate(self._images_prediction_lst):
            image_output_path = os.path.join(output_folder, f"pred_{i}.jpg")
            prediction.save(output_path=image_output_path, box_thickness=box_thickness, show_confidence=show_confidence,
...
class VideoDetectionPrediction(VideoPredictions):
    ...
    def draw(self, box_thickness: int = 2, show_confidence: bool = True,
             color_mapping: Optional[List[Tuple[int, int, int]]] = None) -> List[np.ndarray]:
        ...
        """
        frames_with_bbox = [
            result.draw(box_thickness=box_thickness, show_confidence=show_confidence, color_mapping=color_mapping) for
            result in self._images_prediction_lst
        ]
