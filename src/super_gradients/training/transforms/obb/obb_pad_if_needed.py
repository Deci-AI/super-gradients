...
from super_gradients.training.transforms.utils import _pad_image, PaddingCoordinates, _shift_bboxes_cxcywhr

from .abstract_obb_transform import AbstractOBBDetectionTransform


@register_transform()
class OBBDetectionPadIfNeeded(AbstractOBBDetectionTransform):
    """
    ...
    def apply_to_sample(self, sample: OBBSample) -> OBBSample:
        ...
        return OBBSample(
            ...
            rboxes_cxcywhr=_shift_bboxes_cxcywhr(sample.rboxes_cxcywhr, pad_left, pad_top),
    ...
    def get_equivalent_preprocessing(self) -> List:
        if self.padding_mode == "bottom_right":
            return [{Processings.OBBDetectionBottomRightPadding: {"output_shape": (self.min_height, self.min_width), "pad_value": self.image_pad_value}}]
        elif self.padding_mode == "center":
            return [{Processings.OBBDetectionCenterPadding: {"output_shape": (self.min_height, self.min_width), "pad_value": self.image_pad_value}}]
        else:
            raise RuntimeError(f"OBBDetectionPadIfNeeded with padding_mode={self.padding_mode} is not implemented.")
