...
from super_gradients.training.datasets.data_formats.bbox_formats.normalized_cxcywh import normalized_cxcywh_to_xyxy
...
@register_loss(Losses.DECIYOLO_LOSS)
class DeciYoloLoss(nn.Module):
    def __init__(
        self,
        num_classes: int,
    ...
    def _yolox_targets_to_ppyolo(self, targets: torch.Tensor, batch_size: int) -> Mapping[str, torch.Tensor]:
        ...
        :param targets: (N, 6) format of bboxes is meant to be LABEL_NORMALIZED_CXCYWH (index, c, cx, cy, w, h)
        ...
        gt_bbox = normalized_cxcywh_to_xyxy(targets[:, 2:6], image_shape=self.image_shape)
