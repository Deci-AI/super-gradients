...
class YoloXDetectionLoss(_Loss):
    ...
    where:
        ...
        * L_objectivness is calculated for all cells.
        ...
        L_l1:
            for cells that have suitable ground truths in their grid locations
            l1 distance between the logits and GTs in “logits” format (the inverse of “logits to tensor” ops)
            Coef: 1[use_l1]
        L_objectness:
    ...
    def forward(self, model_output: Union[list, Tuple[torch.Tensor, List]], targets: torch.Tensor):
        ...
        if isinstance(model_output, tuple) and len(model_output) == 2:
            # in test/eval mode the Yolo model outputs a tuple where the second item is the raw tensor
    ...
    def prepare_predictions(self, predictions: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        :param predictions:     output from all Yolo levels, each of shape
                                [Batch x 1 x GridSizeY x GridSizeX x (4 + 1 + Num_classes)]
        :return:    5 tensors representing tensor:
                        ...
                        * transformed_outputs: shape [batch_size x num_cells x (num_classes + 5)],
                          tensor with boxes in real coordinates and logprobabilities
                        * raw_outputs: shape [batch_size x num_cells x (num_classes + 5)],
                          raw tensor with boxes and confidences as logits
        ...
        for k, output in enumerate(predictions):
            ...
            # x cell coordinates of all 784 tensor, 0, 0, 0, ..., 1, 1, 1, ...
            x_shifts.append(grid_raveled[:, :, 0])
            # y cell coordinates of all 784 tensor, 0, 1, 2, ..., 0, 1, 2, ...
    ...
    def get_assignments(
        ...
        :param cls_preds: torch.Tensor: Class tensor in all cells, shape (batch_size, num_cells).
        :param obj_preds: torch.Tensor: Objectness tensor in all cells, shape (batch_size, num_cells).
...
class YoloXFastDetectionLoss(YoloXDetectionLoss):
    ...
    * There are differences in terms of the algorithm itself:
    1. When computing a dynamic k for a ground truth,
        in the original implementation they consider the sum of top 10 tensor sorted by ious among the initial
    ...
    def _compute_matching(
        ...
        :param bbox_preds: tensor of bounding boxes. shape [batch, n_anchors_all, 4]
        :param cls_preds:  tensor of class.          shape [batch, n_anchors_all, n_cls]
        :param obj_preds:  tensor for objectness.    shape [batch, n_anchors_all, 1]
