...
class YoloNASRLoss(nn.Module):
    ...
    def _rbox_loss_v2(
        ...
        if self.dfl_loss_weight > 0:
            ...
            loss_dfl = torch.tensor([0], device=pred_bboxes.device, dtype=torch.float32)
        ...
        if self.size_loss_weight > 0:
            ...
            loss_l1_size = torch.tensor([0], device=pred_bboxes.device, dtype=torch.float32)
        ...
        if self.centers_loss_weight > 0:
            ...
            loss_l1_centers = torch.tensor([0], device=pred_bboxes.device, dtype=torch.float32)
