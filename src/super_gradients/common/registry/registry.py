...
class Registry:
    def __init__(self, default_items: Optional[Dict[str, Callable]] = None):
    ...
    def register(self, name: Optional[str] = None) -> Callable:
        ...
        def decorator(cls: Callable) -> Callable:
            ...
            if cls_name in self.items:
                ref = self.items[cls_name]
                raise Exception(f"`{cls_name}` is already registered and points to `{inspect.getmodule(ref).__name__}.{ref.__name__}")

            self.items[cls_name] = cls
            return cls

        return decorator


def create_register_decorator(registry: Optional[Dict[str, Callable]] = None) -> Callable:
    """
...
# register_loss = create_register_decorator()
