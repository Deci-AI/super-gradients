...
detection_compile_tests_1gpu:
	CUDA_VISIBLE_DEVICES=0 python -m super_gradients.train_from_recipe --config-name=coco2017_yolo_nas_s batch_size=16 experiment_name=coco2017_yolo_nas_s_compile_enabled_1gpu  training_hyperparams.torch_compile=True  $(SINGLE_GPU)    $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS) &
	CUDA_VISIBLE_DEVICES=1 python -m super_gradients.train_from_recipe --config-name=coco2017_yolo_nas_s batch_size=16 experiment_name=coco2017_yolo_nas_s_compile_disabled_1gpu training_hyperparams.torch_compile=False $(SINGLE_GPU)    $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS) &

	CUDA_VISIBLE_DEVICES=2 python -m super_gradients.train_from_recipe --config-name=coco2017_yolo_nas_m batch_size=12 experiment_name=coco2017_yolo_nas_m_compile_enabled_1gpu  training_hyperparams.torch_compile=True  $(SINGLE_GPU)    $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS) &
	CUDA_VISIBLE_DEVICES=3 python -m super_gradients.train_from_recipe --config-name=coco2017_yolo_nas_m batch_size=12 experiment_name=coco2017_yolo_nas_m_compile_disabled_1gpu training_hyperparams.torch_compile=False $(SINGLE_GPU)    $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS) &

	CUDA_VISIBLE_DEVICES=4 python -m super_gradients.train_from_recipe --config-name=coco2017_yolo_nas_l batch_size=8 experiment_name=coco2017_yolo_nas_l_compile_enabled_1gpu  training_hyperparams.torch_compile=True  $(SINGLE_GPU)    $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS) &
...
detection_compile_tests_compile_loss_1gpu:
	CUDA_VISIBLE_DEVICES=0 python -m super_gradients.train_from_recipe --config-name=coco2017_yolo_nas_s batch_size=16 experiment_name=coco2017_yolo_nas_s_compile_loss_enabled_1gpu   training_hyperparams.torch_compile=True training_hyperparams.torch_compile_loss=True  $(SINGLE_GPU) $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS)
...
#	python -m super_gradients.train_from_recipe --config-name=imagenet_resnet50 batch_size=128 experiment_name=imagenet_resnet50_compile_disabled_ddp  training_hyperparams.torch_compile=False $(MULTIPLE_GPUS) $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS)
	...
	python -m super_gradients.train_from_recipe --config-name=imagenet_vit_base batch_size=16 experiment_name=imagenet_vit_base_compile_enabled_ddp   training_hyperparams.torch_compile=True  $(MULTIPLE_GPUS) $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS)
	python -m super_gradients.train_from_recipe --config-name=imagenet_vit_base batch_size=16 experiment_name=imagenet_vit_base_compile_disabled_ddp  training_hyperparams.torch_compile=False $(MULTIPLE_GPUS) $(DEFAULT_TRAINING_PARAMETERS) $(LOGGING_PARAMETERS)
