...
class DiceLossTest(unittest.TestCase):
    ...
    def _assertion_dice_torch_values(self, expected_value: torch.Tensor, found_value: torch.Tensor, rtol: float = 1e-5):
        self.assertTrue(
            ...
            msg=f"Unequal dice loss: excepted: {expected_value}, found: {found_value}"
        )
    ...
    def test_dice(self):
        ...
        expected_dice_loss = 1. - ((2. * intersection) / (denominator + self.eps))
    ...
    def test_generalized_dice(self):
        ...
        eps = 1e-17
        expected_dice_loss = 1. - ((2. * weights * intersection) / (weights * denominator + eps))
        expected_dice_loss = expected_dice_loss.mean()
