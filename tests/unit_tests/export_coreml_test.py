...
class TestModelsCoreMLExport(unittest.TestCase):
    def test_models_onnx_export_with_deprecated_input_shape(self):
        ...
        preprocess = Compose(
            [Resize(224), Standardize(), Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
        with tempfile.TemporaryDirectory() as tmpdirname:
            out_path = os.path.join(tmpdirname, "resnet18.mlmodel")
            models.convert_to_coreml(model=pretrained_model, out_path=out_path, input_shape=(3, 256, 256),
    ...
    def test_models_coreml_export(self, **export_kwargs):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            model_path = models.convert_to_coreml(
                model=pretrained_model, out_path=out_path, pre_process=preprocess,
                prep_model_for_conversion_kwargs=dict(input_size=(1, 3, 640, 640)),
                **export_kwargs
            )
            self.assertTrue(os.path.exists(model_path))
            self.assertTrue(
                model_path.endswith(".mlprogram" if export_kwargs.get('export_as_ml_program') else ".mlmodel"))
