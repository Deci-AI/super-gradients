...
class QuantizationUtilityTest(unittest.TestCase):
    def test_vanilla_replacement(self):
        ...
        self.assertTrue(
            isinstance(module.conv1, SelectiveQuantizer.mapping_instructions[nn.Conv2d].quantized_target_class))
    ...
    def test_nested_module_replacement(self):
        ...
        self.assertTrue(isinstance(module.conv,
                                   SelectiveQuantizer.mapping_instructions[nn.Conv2d].quantized_target_class))
        self.assertTrue(isinstance(module.my_block.linear,
                                   SelectiveQuantizer.mapping_instructions[nn.Linear].quantized_target_class))
    ...
    def test_custom_quantized_mapping_wrapper_explicit_from_float(self):
        ...
        self.assertTrue(
            isinstance(module.conv, SelectiveQuantizer.mapping_instructions[nn.Conv2d].quantized_target_class))
    ...
    def test_different_quant_descriptors_are_piped(self):
        ...
        # TEST
        q_util = SelectiveQuantizer()
        q_util.register_quantization_mapping(layer_names={'conv1'}, quantized_target_class=QuantConv2d,
                                             input_quant_descriptor=QuantDescriptor(calib_method='max'),
                                             weights_quant_descriptor=QuantDescriptor(calib_method='histogram'))
        q_util.register_quantization_mapping(layer_names={'conv2'}, quantized_target_class=QuantConv2d,
    ...
    def test_torchvision_resnet_sg_vanilla_quantization_matches_pytorch_quantization(self):
        ...
        sq = SelectiveQuantizer(custom_mappings={
            torch.nn.Conv2d:
                QuantizedMetadata(torch.nn.Conv2d, quant_nn.QuantConv2d,
            ...
            torch.nn.Linear:
                QuantizedMetadata(torch.nn.Linear, quant_nn.QuantLinear,
                                  action=QuantizedMetadata.ReplacementAction.REPLACE_AND_DONT_QUANTIZE_CHILD_MODULES,
                                  input_quant_descriptor=QuantDescriptor(calib_method='histogram'),
                                  weights_quant_descriptor=QuantDescriptor(calib_method='max', axis=0)),
            torch.nn.AdaptiveAvgPool2d:
                QuantizedMetadata(torch.nn.AdaptiveAvgPool2d, quant_nn.QuantAdaptiveAvgPool2d,
                                  action=QuantizedMetadata.ReplacementAction.REPLACE_AND_DONT_QUANTIZE_CHILD_MODULES,
                                  input_quant_descriptor=QuantDescriptor(calib_method='max'))
    ...
    def test_sg_resnet_sg_custom_quantization_matches_pytorch_quantization(self):
        ...
        sq = SelectiveQuantizer(custom_mappings={
            Bottleneck:
                QuantBottleneck.get_quantized_metadata_for_source(Bottleneck),
            torch.nn.Conv2d:
                QuantizedMetadata(torch.nn.Conv2d, quant_nn.QuantConv2d,
                                  action=QuantizedMetadata.ReplacementAction.REPLACE_AND_DONT_QUANTIZE_CHILD_MODULES,
                                  input_quant_descriptor=QuantDescriptor(calib_method='histogram'),
        ...
        resnet_pyquant = utils.quantization.quantized_modules.pytorch_quantization_resnet.ResNet(
            utils.quantization.quantized_modules.pytorch_quantization_resnet.Bottleneck,
            [3, 4, 6, 3],
            quantize=True)

        new_ckpt_dict = {}
        for (ckpt_key, ckpt_val), (model_key, model_val) \
                in zip(resnet_sg.state_dict().items(), resnet_pyquant.state_dict().items()):
