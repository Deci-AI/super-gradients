...
from super_gradients.training.utils.fine_grain_quantization_utils import QuantizationUtility, RegisterQuantizedModule
    ...
    def test_custom_quantized_mapping_wrapper_explicit_from_float(self):
        ...
        class MyModel(nn.Module):
            def __init__(self, res, n_classes) -> None:
                super().__init__()
                self.conv = nn.Conv2d(3, 4, kernel_size=3, padding=1)
                self.my_block = QuantizedMapping(MyBlock(4 * (res ** 2), n_classes), MyQuantizedBlock)
