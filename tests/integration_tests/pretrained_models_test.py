...
class PretrainedModelsTest(unittest.TestCase):
    ...
    def test_pretrained_resnet50_imagenet(self):
        trainer = SgModel('imagenet_pretrained_resnet50_new_head', model_checkpoints_location='local',
                          multi_gpu=MultiGPUMode.OFF)
        arch_params = self.imagenet_pretrained_arch_params["resnet50"].copy()
        arch_params['num_classes'] = 10
        trainer.build_model("resnet50", arch_params=arch_params)
