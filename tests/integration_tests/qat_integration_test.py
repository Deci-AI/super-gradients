...
class QATIntegrationTest(unittest.TestCase):
    def _get_trainer(self, experiment_name):
        trainer = Trainer(experiment_name, multi_gpu=MultiGPUMode.OFF)
    ...
    def test_qat_from_start(self):
        ...
        train_params = self._get_train_params(
            qat_params={"start_epoch": 0, "quant_modules_calib_method": "percentile", "calibrate": True, "num_calib_batches": 2, "percentile": 99.99}
        )

        model.train(model=net, training_params=train_params, train_loader=classification_test_dataloader(), valid_loader=classification_test_dataloader())

    def test_qat_transition(self):
        model, net = self._get_trainer("test_qat_transition")
        train_params = self._get_train_params(
            qat_params={"start_epoch": 1, "quant_modules_calib_method": "percentile", "calibrate": True, "num_calib_batches": 2, "percentile": 99.99}
        )
