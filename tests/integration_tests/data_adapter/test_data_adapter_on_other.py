...
def torch_classification():
    ...
    train_loader = DataLoader(
        ...
        batch_size=20,
        num_workers=0,
        collate_fn=ClassificationDatasetAdapterCollateFN(adapter_cache_path=analyzer.data_config.cache_path, n_classes=len(train_set.categories)),
        drop_last=True,
    )
    ClassificationDatasetAdapterCollateFN.setup_adapter(train_loader)
